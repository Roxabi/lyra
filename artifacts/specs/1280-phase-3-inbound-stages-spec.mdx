...
- [ ] `session_builder.py` has zero `import discord` and zero `import aiogram` statements; handles `turn_store=None ∧ thread_store=None` (test/CLI), `turn_store ∧ ¬thread_store` (Telegram), and `turn_store ∧ thread_store` (Discord) without `isinstance(platform_meta, …Meta)` branches in module code (allowed in unit tests only)
...
[NEEDS CLARIFICATION 2] — **Router cold-path I/O**: today Discord routing performs an async `ThreadStore.is_owned` lookup mid-routing (lines 70-87 of `discord_inbound.py`) to warm `_owned_threads` lazily. Options: (a) make Router async so it can do I/O; (b) keep Router synchronous + pure; lift the I/O into the `pre_session_hook` (NC1) — but that runs *after* Router decides PROCESS, so the cold-path warmup must happen *before* Router or as part of it; (c) eager-load `_owned_threads` once at adapter start so Router stays pure; (d) accept that Router is best-effort sync: if it returns DROP based on stale `_owned_threads`, the message is dropped — but the warmup hook fires in a background task to fix future messages. **Recommended: (c)** — eager-load at adapter start makes Router pure, removes the lazy-warmup branch from `handle_message`, and shifts the one-time cost to bootstrap (where it belongs). Falls back to (d) only if eager-load is infeasible (e.g. very large thread sets).

...
> **Status (2026-07-02):** Root cause #1 **fixed** on branch `fix/astryx-padding-cascade` (issue #2148) — the universal reset was relocated from `@layer brand` to `@layer reset`; verified locally (Astryx Button padding `0`→`8px 12px`, no regressions on home/jobs/agents/fleet). Root causes #2 (Table auto-layout collapse), #3 (`--color-on-accent`), #4 (bordered variant) remain as follow-ups (see #2148). This document is the evidence trail; `apps/dashboard/DESIGN.md` is the forward-looking doctrine.
...
- **No visual-regression gate anywhere in S0–S12.** The cutover commit (S12, `4248c0e5`) asserts the token/utility CSS layer is "byte-identical" to justify removing the shadcn bridge — true for tokens, but the actual regressions came from the S1–S11 **component** swaps, which nothing checked pixel-for-pixel.

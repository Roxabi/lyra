...
Add structured persona config ("soul file") stored in roxabi-vault (`~/.roxabi-vault/personas/`) to Lyra agents, auto-compose natural prose system prompts, and fix the `--system-prompt` CLI injection gap. Separates customization from engine code. 3 slices, 18 micro-tasks.

## Architecture

```mermaid
flowchart TD
    subgraph "Config Files (src/lyra/agents/)"
        TOML["lyra_default.toml\n[agent].persona = 'lyra_default'"]
    end

    subgraph "Vault (~/.roxabi-vault/personas/)"
    ...
    TOML -->|"reads persona name"| LAC
    LAC -->|"resolves name → vault path"| LP
    PERSONA -->|"parsed"| LP
    LP --> PC
...
```mermaid
flowchart LR
    subgraph "~/.roxabi-vault/personas/"
    ...
    subgraph "src/lyra/core/agent.py"
        ...
        FN2["load_persona(name, personas_dir) → PersonaConfig"]
...
|-------|-----------|-------|
| backend-dev | 13 | vault/personas/persona.toml, agent.py, cli_pool.py, simple_agent.py, lyra_default.toml |
...
```python
_VAULT_DIR = Path(os.environ.get("ROXABI_VAULT_DIR", Path.home() / ".roxabi-vault"))
...
- Add `import os` to agent.py
- Validate name with `[a-zA-Z0-9_-]` regex (same guard as agent names)
- Default `personas_dir` to `_PERSONAS_DIR` (derived from `ROXABI_VAULT_DIR` env var, fallback `~/.roxabi-vault/personas/`)
- Ensure `personas_dir` exists (`mkdir(parents=True, exist_ok=True)`)
- Resolve `{personas_dir}/{name}.persona.toml`
...
**Verify:** `uv run python -c "from lyra.core.agent import load_persona; p = load_persona('lyra_default'); print(p.identity.name)"`
**Expected:** `Lyra`

#### Task 1.3 — Create persona TOML file in vault [P]
**Agent:** backend-dev
**File:** `~/.roxabi-vault/personas/lyra_default.persona.toml` (NEW)
**Spec trace:** SC-1
**Phase:** GREEN
**Difficulty:** 1

Create the vault `personas/` directory and the file with all 4 sections matching the spec breadboard schema. Include `goal` field in `[identity]`.

**Verify:** `uv run python -c "import tomllib; f=open('$HOME/.roxabi-vault/personas/lyra_default.persona.toml','rb'); d=tomllib.load(f); print(d['identity']['name'], d['identity']['goal'])"`
...
**Agent:** backend-dev
**File:** `src/lyra/core/agent.py`
**Spec trace:** SC-8, SC-9, SC-10, SC-11, SC-6, Edge: no persona, both present, name traversal
**Phase:** RED
**Difficulty:** 4

- Read `[agent].persona` if present (name string, not a path)
- Call `load_persona(name)` → resolves to vault path → store on `Agent.persona`
...
- No `persona` key → `Agent.persona = None`, `[prompt].system` used unchanged
...
- Extend `_maybe_reload()` in `AgentBase` to track persona file mtime (vault path)
...
#### Task 3.5 — Update `lyra_default.toml` — add persona name, remove raw prompt
**Agent:** backend-dev
...
- Add `persona = "lyra_default"` to `[agent]` section
- Remove `[prompt].system` (persona auto-composes now from vault file)
...
Three test cases (use tmp_path for personas_dir in tests):
1. Agent TOML with `persona` only → composed prompt, persona populated from vault
2. Agent TOML with `persona` + `[prompt].system` → raw prompt wins, persona still loaded
3. Agent TOML without `persona` → `persona is None`, raw prompt used

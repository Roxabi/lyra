...
def test_generate_request_subject() -> None:
    assert SUBJECTS.generate_request == "factory.llm.generate.request"


def test_heartbeat_subject() -> None:
    assert SUBJECTS.heartbeat == "factory.llm.heartbeat"
...
def test_per_worker_helper() -> None:
    assert per_worker_llm("w1") == "factory.llm.generate.request.w1"

...
5. On subprocess spawn, `_spawn()` merges identity env vars into the subprocess env **via an explicit post-filter merge step** — *not* by extending `_SAFE_ENV_KEYS`. The allowlist is reserved for vars inherited from the parent process environment (its design intent per the inline rationale at `cli_pool_worker.py:36–54`); synthesised-at-spawn vars take a separate codepath. The gate is **"both `agent_name` and `agent_email` present"** (the optional identity pair). When the pair is present, all four vars are merged: `GIT_COMMITTER_NAME`, `GIT_COMMITTER_EMAIL`, `LYRA_SESSION_ID`, `LYRA_AGENT`. When the pair is absent (legacy hub or unbound bot), no identity vars are injected and the subprocess falls back to the template identity. Trailer-only partial mode (inject `LYRA_*` without `GIT_COMMITTER_*`) is intentionally **not** supported in this slice — committer identity and trailers are coupled to keep one reviewer-visible source of attribution; partial mode is future work if a use case emerges.
...
| Agent profile row missing `email` | Hub publishes `agent_name` only; `_spawn` guard fails the "agent_name AND agent_email both present" pair check → fallback. Hub logs `WARN agent missing email`. |
...
- [ ] When hub publishes a `CliCmdPayload` for a bot bound to agent `X` (with email `e`), the payload carries `agent_name="X"` and `agent_email="e"`.
- [ ] When the worker dispatches a `CliCmdPayload` with both `agent_name` and `agent_email` present (and the always-present `lyra_session_id`), the resulting subprocess environment contains `GIT_COMMITTER_NAME`, `GIT_COMMITTER_EMAIL`, `LYRA_SESSION_ID`, `LYRA_AGENT` with matching values.
- [ ] When either `agent_name` or `agent_email` is missing/None, no `GIT_COMMITTER_*` or `LYRA_*` vars are injected and the subprocess spawns successfully.
...
- [ ] **End-to-end attribution check**: a commit produced through the full hub→clipool→subprocess path with agent `X` (email `e`) and session id `S` yields, when queried via `git log -1 --format='%cn|%ce|%(trailers:key=Lyra-Session-Id,valueonly)|%(trailers:key=Lyra-Agent,valueonly)'`, the exact line `X|e|S|X`. This is the reviewer-visible signal the frame's failure mode is testing against.

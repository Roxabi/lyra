# src/factory/inbound/ — Stage-Axis Inbound Pipeline
...
- `AttachmentIngestStage` — central inbound attachment ingest (→ `docs/architecture/adapters.md` § Inbound stage pipeline, epic #1537). Runs immediately after parse, **before** routing. When a store is configured, uploads binary attachments on the `InboundMessage` to `BlobStore`, replacing raw bytes with `BlobRef` values so downstream stages (dispatcher, agents) receive opaque refs only. Failures degrade gracefully (`blob_ref=None`). When no store is configured, the stage is skipped; instead, any `pending_attachment` (singular, audio #1551) and `pending_attachments` (plural, non-audio #1552) closures are cleared via `dataclasses.replace` — a closure must never cross the NATS process boundary (transport-boundary invariant).
...
- Adapter-side hooks live in `src/factory/adapters/{platform}/{platform}_inbound.py` because
...
- ¬ move Discord thread API helpers (`persist_thread_claim`) here — they import `discord` and stay in `adapters/discord/discord_threads.py`. The thread-session write-back helpers were removed in #1777; last-session now resolves solely via `turns.db` path-3 (`get_last_session`).

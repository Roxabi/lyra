...
The satellite additionally enforces a **conservative base64 ceiling of 750 KB** in `imagecli/src/imagecli/nats/adapter.py` (file-write block in `handle()` + helpers `_nats_output_dir` / `_move_to_persistent_output`):

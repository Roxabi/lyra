...
flowchart TD
    ...
    MTX -->|factory-acl genkeys --regenerate| RND --> AUTH
    SUB --> NLC
    ENV --> NLC
    OV --> NLC
    WH -.annotates.-> NLC
...
- ACL source-of-truth is `deploy/nats/acl-matrix.json` (JSON, not YAML as
  the issue body says). `factory-acl genkeys --regenerate` deterministically
...
```bash
uv run factory-acl genkeys --template-only > deploy/nats/auth.conf.new
...
| T2 | devops-A | T1 | Regenerate auth.conf via factory-acl genkeys --template-only |
...
- T1: 12 — Edit acl-matrix.json: flip hub + llm-worker subjects to canonical
- T2: 13 — Regenerate auth.conf via factory-acl genkeys
- T3: 14 — nats_llm_client.py: drop per_worker_llm, publish on SUBJECTS.generate_request
- T4: 15 — llm_overlay.py: instantiate NatsLlmClient instead of NatsLlmDriver
- T5: 16 — Type annotations: NatsLlmDriver → NatsLlmClient across 4 wiring files
- T6: 17 — Add tests/scripts/test_acl_canonical_llm.py — 3 regression assertions
- T7: 18 — Update fixture files + test_ops_verify.py legacy refs
- T8: 19 — Migrate test_nats_driver.py → test_nats_llm_client.py with envelope tests
- T11: 20 — gh issue create — follow-up removal issue for legacy NatsLlmDriver
- T10: 21 — nats_driver.py: add # DEPRECATED header citing T11 issue number
- T12: 22 — RED-GATE: atomic verify (5 checks)

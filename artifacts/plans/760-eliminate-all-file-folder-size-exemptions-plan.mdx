...
**Files still over 300 lines:** hub.py (542), _shared_streaming.py (478), cli_pool.py (406), hub_standalone.py (362), outbound_router.py (324)

**Commit:** feat/760-eliminate-all-file-folder-size-exemptions (rebased)
...
#### Task P1: Update stale exemption files → devops
...
#### Task P2: Measure coverage baseline → tester
...
#### Task 1.1: Extract StreamingState from _shared.py [P] → backend-dev
...
#### Task 1.2: Extract BotAgentMapStore from agent_store.py [P] → backend-dev
...
#### Task 1.3: Extract DiscordLifecycle from discord.py [P] → backend-dev
...
#### Task 1.4: Extract CliStreamingParser from cli_streaming.py [P] → backend-dev
...
#### Task 1.5: Extract TurnStoreSession from turn_store.py [P] → backend-dev
...
#### Task 1.6: Refactor command_router.py dispatch → backend-dev
...
#### RED-GATE: V1 verification → tester
...
#### Task 2.1: Extract TtsDispatch from outbound_router.py → backend-dev
...
#### Task 2.2: Extract AudioDispatch from outbound_router.py → backend-dev
...
#### Task 2.3: Extract NatsAdapterWiring from hub_standalone.py → backend-dev
...
#### Task 2.4: Extract AuthSeeding from hub_standalone.py [P] → backend-dev
...
#### Task 2.5: Extract HubBuilder from hub_standalone.py [P] → backend-dev
...
#### Task 2.6: Extract CliPoolSession from cli_pool.py [P] → backend-dev
...
#### Task 2.7: Extract CliPoolStreaming from cli_pool.py [P] → backend-dev
...
#### Task 2.8: Extract CliPoolLifecycle from cli_pool.py [P] → backend-dev
...
#### RED-GATE: V2 verification → tester
...
#### Task 3.1: Extract HubShutdown from hub.py → backend-dev
...
#### Task 3.2: Extract HubCircuitBreaker from hub.py [P] → backend-dev
...
#### Task 3.3: Extract HubPoolDelegation from hub.py [P] → backend-dev
...
#### Task 3.4: Extract HubDispatch from hub.py [P] → backend-dev
...
#### RED-GATE: V3 verification → tester
...
#### Task 4.1: Create subfolders under core/ → devops
- **File:** `src/lyra/core/`
...
#### Task 4.2: Create subfolders under adapters/ → devops
...
#### Task 4.3: Create subfolders under bootstrap/ → devops
...
#### RED-GATE: Final verification → tester

...
**Governance prerequisite (Slice 0):** `src/lyra/infrastructure/CLAUDE.md` mandates an ADR for any new subdirectory under `infrastructure/`. Slice 0 adds **ADR-075 — TurnWriter subscriber-writer sublayer** before any code lands under `infrastructure/turn_writer/`. Without this, the spec violates the governance rule.
...
```
A0 (ADR-075) authorises infrastructure/turn_writer/ — prerequisite to S3/S7
...
|---|---|---|---|
| 0 | **Governance** — write ADR-075 authorising `infrastructure/turn_writer/` sublayer; merge before any code below | A0 | `ls docs/architecture/adr/075-turn-writer-subscriber.mdx` + `status: accepted` |
...
- [ ] ADR-075 merged (`docs/architecture/adr/075-turn-writer-subscriber.mdx` with `status: accepted`) before Slice 2 lands.

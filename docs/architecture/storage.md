...
- JetStream KV `factory-state` carries hub readiness (`hub.ready`, `roxabi_nats/readiness.py`) and adapter bot roster mirrors (`roster.telegram`, `roster.discord` — see `messaging.md`). It is NOT a session store.

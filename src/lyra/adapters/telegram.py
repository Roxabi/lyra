...
from lyra.adapters.telegram_audio import (
    _download_audio as _download_audio_fn,
)
from lyra.adapters.telegram_audio import (
    _on_voice_message as _on_voice_message_fn,
)
from lyra.adapters.telegram_audio import (
    normalize_audio as _normalize_audio_fn,
)
from lyra.adapters.telegram_audio import (
    render_audio as _render_audio_fn,
)
from lyra.adapters.telegram_audio import (
    render_audio_stream as _render_audio_stream_fn,
)
from lyra.adapters.telegram_audio import (
    render_voice_stream as _render_voice_stream_fn,
)
...
from lyra.adapters.telegram_formatting import (
    render_attachment as _render_attachment_fn,
)
...
class TelegramAdapter:
    ...
    async def _download_audio(
        ...
        return await _download_audio_fn(self, file_id, duration)

    async def _on_voice_message(self, msg) -> None:
        """Handle an incoming voice or audio message."""
        await _on_voice_message_fn(self, msg)
    ...
    def _render_text(self, text: str) -> list[str]:
        """Thin wrapper — delegates to telegram_formatting._render_text."""
        return _render_text(text)

    def _render_buttons(self, buttons: list) -> object | None:
        """Thin wrapper — delegates to telegram_formatting._render_buttons."""
    ...
    async def render_audio(self, msg: OutboundAudio, inbound: InboundMessage) -> None:
        ...
        await _render_audio_fn(self.bot, msg, inbound)
    ...
    async def render_attachment(
        ...
        await _render_attachment_fn(self.bot, msg, inbound)
    ...
    async def render_audio_stream(
        ...
        await _render_audio_stream_fn(self, chunks, inbound)
    ...
    async def render_voice_stream(
        ...
        await _render_voice_stream_fn(chunks, inbound)

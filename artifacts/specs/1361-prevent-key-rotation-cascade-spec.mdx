...
3. The Makefile target chains:
   - `uv run --project <repo> factory-acl genkeys --add-identity turn-writer` — generates `~/.lyra/nkeys/turn-writer.seed` only (if absent), then re-renders `~/.lyra/nkeys/auth.conf` using existing pubkeys for every other active identity (read from existing seeds, never regenerated). Emits `STATE=noop|repaired|added` on stdout.
...
- **On the authoring host (today: M₁):** factory-acl emits `STATE=added`; Phase 2 creates the seed + auth.conf secrets and restarts the local NATS server + adapters.
- **On a receiving host (today: M₂; future: M₃, Mₙ):** Syncthing has already delivered the seed and user-mirror `auth.conf` → factory-acl emits `STATE=noop`. But `podman secret inspect lyra-nats-<x>` returns non-zero (secret missing locally) → Phase 2 still runs to create the local Podman secret + restart whatever local units consume it (via the `is-active`-gated loop).
- **On a non-consuming host:** factory-acl `STATE=noop` AND `podman secret inspect lyra-nats-<x>` succeeds (or no Lyra units run there at all → loop is a no-op).

The Makefile gating means the verb is **idempotent on every host** and **safe to run anywhere** without operator knowledge of the topology. The factory-acl tool owns filesystem state (`STATE=noop|repaired|added`); the Makefile owns local Podman store state (`podman secret inspect`). These two concerns are orthogonal — factory-acl never shells out to Podman.
...
- A "Multi-host: running on M₂ / Mₙ" subsection documents that the **same verb** is run on every consuming host (no separate procedure), explains the factory-acl-STATE × local-Podman-secret gate so operators understand why it is safe, and notes that Syncthing handles seed/auth.conf propagation.
- A "Recovery — Phase 2 fails after factory-acl succeeds" subsection documents the safe retry path: re-run `make nats-add-identity NAME=<x>` — idempotent on the filesystem side, `--replace` on the Podman side makes retry safe.
...
```mermaid
flowchart TB
    M[make nats-add-identity NAME=x<br/>run as operator, no sudo] --> A1[uv run factory-acl genkeys --add-identity x]
...
|---|---|---|---|
| `factory-acl genkeys --add-identity` | reads `acl-matrix.json[identities]`, reads existing `*.seed` files | invocation | this issue |
| `factory-acl genkeys --regen-authconf` | reads all `*.seed`, reads matrix | already exists | unchanged |
| `factory-acl genkeys` (full provision) | reads matrix, writes all `*.seed` | already exists | unchanged |
...
|---|---|---|---|
| U1 | `factory-acl genkeys --add-identity <name>` | new `_mode_add_identity(args)` in `scripts/_modes.py` | matrix (read), `<seeds_dir>/<name>.seed` (write, new), `auth.conf` (write, full re-render with existing+new pubkeys) |
...
| F3 | top-level `Makefile` (alongside existing `nats-regen-authconf`, `nats-setup`, `quadlet-secrets-install` targets) | Add `nats-add-identity` target: invoke `uv run factory-acl genkeys --add-identity $(NAME)` (no sudo), capture STATE from stdout. Phase 2 gate: skip if `STATE=noop` AND `podman secret inspect lyra-nats-$(NAME)` succeeds; otherwise run `podman secret create --replace lyra-nats-$(NAME) ~/.lyra/nkeys/$(NAME).seed` + `podman secret create --replace lyra-nats-auth ~/.lyra/nkeys/auth.conf` + restart loop `for svc in lyra-nats lyra-hub lyra-telegram lyra-discord lyra-clipool; do systemctl --user is-active --quiet $$svc && systemctl --user restart $$svc; done`. Update `.PHONY` line. Target runs as operator (no root anywhere). |
...
| ID | Slice | Files | Demo |
|---|---|---|---|
| S1 | **`--add-identity` CLI verb + tests** | F1, F2, F6 | `uv run pytest tests/unit/test_modes_add_identity.py` green; manual: declare a test identity in matrix, run `sudo factory-acl genkeys --add-identity test-id`, assert only new seed written + auth.conf updated |
...
- [ ] `uv run factory-acl genkeys --add-identity <new-name>` writes exactly one new seed file (`<seeds_dir>/<new-name>.seed`) — verified by SHA-256-comparing every other pre-existing `.seed` byte-for-byte before/after invocation (unit test asserts zero diff)
...
- [ ] `factory-acl genkeys --add-identity <name>` runs successfully as a non-root user (no `_require_root()` exit); unit test asserts this by invoking the mode without elevating
...
- [ ] `make nats-add-identity NAME=<existing>` (full-consistency case, secret already present locally) is a clean no-op chain: factory-acl prints `STATE=noop`, Makefile sees `podman secret inspect` success, skips `podman secret create` AND `systemctl restart` entirely (verified via instrumentation: zero podman/systemctl mutating invocations after the factory-acl call)
...
- [ ] `docs/ops/nats-identity-lifecycle.md` has sections "Adding an identity" (with `make nats-add-identity` example, step 2 rewritten from `--regen-authconf` to the new verb), "Retiring an identity" (existing content preserved verbatim), "Recovery — Phase 2 fails after factory-acl succeeded", and "Multi-host: running on M₁, M₂, Mₙ"
- [ ] All existing `factory-acl` modes (`full provision`, `--regen-authconf`, `--regenerate`, `--show`, `--fix-perms`, `--emit-merged-authconf`) continue to pass their existing tests with zero modification

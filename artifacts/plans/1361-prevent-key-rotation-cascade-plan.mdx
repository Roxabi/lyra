...
flowchart TD
    ...
    subgraph MK["Makefile (operator-owned, no sudo)"]
      M1["uv run lyra-acl genkeys --add-identity $(NAME)"]
...
- Snippet:
  ...
  def test_add_identity_writes_only_new_seed(tmp_path):
      ...
      # Act: subprocess run lyra-acl genkeys --add-identity turn-writer
...
- File: same
- Snippet: ~15-line subsection explaining: same verb on every consuming host; lyra-acl STATE × local Podman secret gate; Syncthing handles seed/auth.conf propagation; restart loop is `is-active`-gated so it's safe everywhere.
...
- Snippet (recovery): "re-run `make nats-add-identity NAME=<x>` — lyra-acl is idempotent on filesystem; `--replace` makes Podman side safe; restart loop is idempotent."

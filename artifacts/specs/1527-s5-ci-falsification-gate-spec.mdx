...
A CI gate, `lyra-acl check grants`, that fails the build when any provisioner or consumer-group
...
`uv run lyra-acl check grants --matrix deploy/nats/acl-matrix.json --code-subjects deploy/nats/code-subjects.json`
...
| `ci.yml` | `lyra-acl check grants` exit code | every PR | new (this issue) |
...
|---|---|---|---|
| C1 | `lyra-acl check grants --matrix P --code-subjects P` | `_cmd_check_grants` (gen_nkeys.py) → `check_grants.run()` | matrix + manifest |
| C2 | `lyra-check-grants` (pyproject script alias) | `gen_nkeys._check_grants` → `main(["check","grants",…])` | — |
| C3 | CI step "Check ACL code coverage (ADR-079)" | `uv run lyra-acl check grants …` | exit 0/1 |
...
| M7 | `.github/workflows/ci.yml` (edit) | new step after "Check request-reply flows" | `uv run lyra-acl check grants --matrix … --code-subjects …` |
...
| S5.2 | `check_grants.py` + manifest + CLI wiring + pyproject alias + CI step + tests | `lyra-acl check grants` exits 0 on real matrix; unit tests prove it exits 1 on each removed grant; CI step present |
...
- [ ] `uv run lyra-acl check grants` exits 0 on the committed matrix + manifest and prints an `OK` line with the resource count.

...
Superseded by ADR-049 — voice subjects table, contract_version rationale, heartbeat cadence/TTL rules, and queue group names absorbed into ADR-049 §Supersedes. Normative wire schemas now live in roxabi_contracts.voice.models.
...
2. **NATS request-reply** — `NatsSttClient` / `NatsTtsClient` send JSON payloads to `factory.voice.{stt,tts}.request`; `stt_adapter_standalone.py` / `tts_adapter_standalone.py` consume them inside a satellite process that owns the voicecli import.
...
### Request — `factory.voice.stt.request`
...
### Reply — `factory.voice.stt.request` (success)
...
### Reply — `factory.voice.stt.request` (error)
...
### Request — `factory.voice.tts.request`
...
### Reply — `factory.voice.tts.request` (success)
...
### Reply — `factory.voice.tts.request` (error)
...
### Heartbeats — `factory.voice.{stt,tts}.heartbeat`
...
```json
{
  ...
  "service": "stt_workers",
  "host": "roxabituwer",
  "subject": "factory.voice.stt.request",
...
**Why these subjects.** `factory.voice.<service>.<verb>` follows the project-wide NATS naming convention (ADR-035): subjects encode the namespace (`lyra`), the capability (`voice`), the service (`stt`/`tts`), and the verb (`request`/`heartbeat`). Flat, predictable, greppable.
...
- **ADR-035** — NATS subject naming convention. `factory.voice.*` follows the `<namespace>.<capability>.<service>.<verb>` shape.
...
- Spec: `git show cc05650ee^:artifacts/specs/658-voicecli-nats-decouple-spec.mdx` — parent decoupling work (S1–S5)

...
intended RED state: every test will fail with a non-zero exit code or
a FileNotFoundError.
"""
...
class TestPositiveCase:
    def test_prod_matrix_exits_zero(self) -> None:
        ...
        SC-5 / SC-22: sibling validator must be a drop-in replacement for the bash
        script.
    ...
    def test_v2_prod_fixture_exits_zero(
...
class TestMissingRequesterIdentity:
    def test_missing_requester_exits_nonzero(
        ...
        # verified: removing the requester-existence check causes exit 0 → fails
        """
        ...
        assert result.returncode != 0
        assert (
            "nonexistent-requester" in result.stdout
            or "nonexistent-requester" in result.stderr
        )
...
class TestMissingResponderIdentity:
    def test_missing_responder_exits_nonzero(
        ...
        # verified: removing the responder-existence check causes exit 0 → fails
        """
        matrix = copy.deepcopy(prod_matrix)
        matrix["request_reply_flows"].append(
            {

...
Epic #704 (vault + intel ingestion) already has two first-class foreign children (`Roxabi/roxabi-vault#24`, `Roxabi/roxabi-intel#12`). Without multi-repo support, they remain invisible to the board.
...
1. User edits `lyra-v2-dependency-graph.layout.json`:
   - Sets `meta.repos = ["Roxabi/lyra", "Roxabi/roxabi-vault", "Roxabi/roxabi-intel"]`
...
| S4 | **Multi-repo integration + docs** — add `#704` cross-repo children to the Lyra layout; `README.md` multi-repo example + migration section; add risk note re. `gh api` dep endpoint stability. | Lyra board renders `roxabi-vault#24` and `roxabi-intel#12` as cards with repo badges and cross-repo dep arrows without any `extra_deps` override. | — |
...
- [ ] Lyra board renders `Roxabi/roxabi-vault#24` and `Roxabi/roxabi-intel#12` as cards with a visible repo badge and a cross-repo dep arrow from `Roxabi/lyra#703`, without any `extra_deps` override.

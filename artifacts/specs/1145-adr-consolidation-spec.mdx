...
- [ ] Every file in `docs/architecture/adr/archive/*.mdx` has a `superseded_by:` frontmatter field whose value is one of: (a) a live ADR id (e.g. `ADR-045`); (b) a comma-separated list of live ADR ids (e.g. `"ADR-035, ADR-045"`); (c) a sentinel value indicating no live successor exists, drawn from `{"code reality", "historical (#666 SDK removal)"}`. Sentinels are valid only for ADRs whose decision was overtaken by code change rather than another ADR; force-mapping such ADRs to a thematically adjacent canonical is rejected because it produces misleading pointers (e.g., ADR-012 SDK-driver content does not actually live anywhere in ADR-059).
...
- [ ] Every live ADR's normative content reflects the current codebase reality (verified by per-cluster code↔ADR architect audit). ADRs whose premise the code has moved past are either rewritten to match, partially-superseded with a banner pointing to current truth, or fully archived. **Truth is the code; the live ADR set is its rationale layer, not a parallel narrative.**
- [ ] Live ADR count is incidental — driven by the truth audit, not a target. Expected outcome ~30–45 after this PR; the exact number is whatever survives validation, not a constraint to hit

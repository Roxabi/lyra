...
Convert `TurnStore` mutators to publish `TurnWriteEvent` over `lyra.turns.write`; a single `TurnWriter` subscriber (Quadlet sibling `lyra-turn-writer.container`) consumes and persists. Slice 0 lands ADR-075 (governance), Slices 1–3 land contracts/publisher/writer/rewire, Slice 4 lands Quadlet + bind-mount swap, Slice 5 lands observability.
...
**T1 — Write ADR-075: TurnWriter subscriber-writer sublayer** *(doc-writer-A · judgmental · 5 ops)*
- File: `docs/architecture/adr/075-turn-writer-subscriber.mdx`
...
- Verify: `ls docs/architecture/adr/075-turn-writer-subscriber.mdx && grep -E "^status:\s*accepted" docs/architecture/adr/075-turn-writer-subscriber.mdx`

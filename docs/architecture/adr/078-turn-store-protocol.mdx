...
Companion to ADR-075 (turn-writer subscriber): 075 owns the write-path sublayer (NATS subscriber → `turns.db`), this ADR owns the read-path layering (`TurnStoreProtocol` in core). Metadata corrected 2026-07-01 — this ADR was wrongly marked as superseding 075.

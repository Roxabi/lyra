...
from factory.errors import KeyringError, MissingCredentialsError
...
async def _main(*, _stop: asyncio.Event | None = None) -> None:
    ...
    """
    load_dotenv()
    if not os.environ.get("FACTORY_HEALTH_SECRET"):
        log.warning(
            "FACTORY_HEALTH_SECRET is not set -- /health returns minimal response only"
        )

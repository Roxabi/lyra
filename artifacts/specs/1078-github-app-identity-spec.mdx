...
- [ ] Single GitHub App `lyra-harness` (App ID `3619198`, install_id `129952244`) registered + installed on the Roxabi org. M₁ AND M₂ Quadlet env (`clipool.env`) point to the same `LYRA_GH_APP_ID` and `LYRA_GH_INSTALLATION_ID`. Verifiable: `grep LYRA_GH_APP_ID deploy/quadlet/clipool.env` returns `3619198` on both hosts. App-only — **no** `lyra-bot` machine user account exists or is referenced
  - *Decision 2026-05-06: collapsed from 2 Apps to 1 for solo-dev. Trigger to split: 2nd contributor or compliance-driven audit-log isolation.*
- [ ] App permissions on the App = exactly `contents:write`, `pull-requests:write`, `issues:write`. Verified via `gh api /apps/lyra-harness` showing no other permission keys ⚠ **descoped 2026-05-06 → Roxabi/lyra#1094** — App currently has 17 permissions. Token isolation invariants I-1/I-2/I-3 hold regardless; this is a least-privilege gap, not a token-leak vector. See audit Decision Log.
- [ ] M₁ AND M₂ clipool `Environment=` show the same `LYRA_GH_APP_ID=3619198` and `LYRA_GH_INSTALLATION_ID=129952244`

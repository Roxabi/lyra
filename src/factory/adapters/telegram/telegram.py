...
from factory.adapters.telegram.telegram_normalize import (
    normalize as _normalize_impl,
    normalize_audio as _normalize_audio_impl,
)
from factory.adapters.telegram.telegram_outbound import (
...
class TelegramAdapter(OutboundAdapterBase):
    ...
    def __init__(  # noqa: PLR0913 — DEBT:wiring-bootstrap-deps — DI constructor
        ...
        if _raw_tmp is not None:
            ...
            if not _tmp_path.is_dir():
                raise RuntimeError(
                    f"FACTORY_AUDIO_TMP={_raw_tmp!r} does not exist "
                    "or is not a directory"
                )
            if not os.access(_raw_tmp, os.W_OK):
                raise RuntimeError(
                    f"FACTORY_AUDIO_TMP={_raw_tmp!r} is not writable by the current"
                    " process"
                )
    ...
    def _make_emitter(
        ...
        from factory.adapters.shared._emitter import _make_emitter as _shared
        from factory.adapters.telegram.telegram_formatter import TelegramFormatter
        from factory.adapters.telegram.telegram_formatting import _validate_inbound
        from factory.adapters.telegram.telegram_outbound import TelegramTypingIndicator

...
## Design Note — four judgment calls (read before Breadboard)
...
**Known limitation, documented not fixed:** 13 of 18 active identities have `allow_responses: true` (verified via direct computation; the 5 with `false` — `telegram-adapter`, `discord-adapter`, `dashboard-reader`, `gh-helper`, `ingress` — never appear as a flow requester or responder). Every one of the 9 flows' responders has `allow_responses: true`. N4 (positive round-trip) can therefore succeed via the dynamic `allow_responses` grant even if a flow's explicit static `_inbox.<requester>.>` injected grant were hypothetically removed — N4 does not isolate the static grant's contribution from the dynamic one. The postmortem's own "Fix 3" §8 (an `allow_responses`-removed fixture) is the remedy for that and is correctly out of scope here.

**Live gap found during implementation, documented not fixed (out of scope for #2247):** `nats-server` 2.10.22 treats an explicit empty allow list (`publish: { allow: [] }` or `subscribe: { allow: [] }`) as **unrestricted**, not deny-all — confirmed by isolated reproduction against a real server (a user permissions block replaces `default_permissions` wholesale rather than merging with it, and an empty allow list leaves that direction's allow-set nil). In `deploy/nats/acl-matrix.json` today this affects `dashboard-reader` (publish: `[]`), `gh-helper` (subscribe: `[]`), and `ingress` (subscribe: `[]`) — each has an unintentionally unrestricted direction, opposite of the documented least-privilege intent. N2/N3 (`test_acl_matrix_publish`/`test_acl_matrix_subscribe`) discovered this on first run: their deny-probe assertion, correct per spec, fails for exactly these three identity/direction pairs, deterministically, independent of the `_probe()` timing race `_settle_deny` exists to absorb (ruled out by re-checking after `nc.drain()` with zero prior probes on the connection). Disposition: `pytest.mark.xfail(strict=True)`, computed from `EFFECTIVE_GRANTS` (not hardcoded identity names, so any future empty-allow-list identity is automatically caught the same way) — `strict=True` means the moment the underlying renderer bug is fixed, the probe starts passing, XPASS fails the suite, and the marker must be removed. Root-cause fix belongs in `scripts/_renderer.py` (e.g. emit an explicit `deny: [">"]` for a direction whose intended allow-list is empty), which is out of scope here per this spec's Out of Scope section (`_renderer.py`/`acl-matrix.json` reused as-is); flagged prominently in the PR description for a human-filed follow-up issue (no unilateral `gh issue create` — Safety Rule).
...
**4. `_settle_deny`'s bounded follow-up poll is an as-built deviation from this spec's original "no bespoke timing budget" wording, amended here rather than in code.** Expected Behavior item 5 and the Edge Cases table originally stated that N5 (and, by the same idiom, N2/N3) is timed solely by reused `_probe`'s own `flush(timeout=2)`. Implementation surfaced 3/68 flaky nodes from the delayed-`error_cb`-delivery race referenced in the paragraph above (the race `_settle_deny` absorbs — distinct from the empty-allow-list gap): a single post-flush yield was not always enough for the violation to have landed by the time `errors` is inspected. The shipped fix, `_settle_deny`, adds a bounded extra poll (5 × 0.2s = up to 1s, only entered when the immediate check reported "accepted" — a probe that already observes the denial pays no extra cost) on top of `_probe`'s own flush window, for every deny-probe across N2, N3, and N5 alike. This is a real, intentional bespoke budget, not a bug — recorded here the same way the `GITHUB_ACTIONS` vs `CI` judgment call below is recorded as a deliberate departure from precedent rather than left implicit in the diff. Expected Behavior item 5 and the Edge Cases N4/N5 row are updated accordingly.
...
| N5 | Non-flow-pair denial (negative, one-way publish check — not a round-trip) | same collection | responder `Y` (connected via reused `_identity_connection`) attempts to publish into `_inbox.<T>.<token>` for a non-flow-paired identity `T`, via reused `_probe(..., expect_deny=True)`; assert a captured permissions-violation error (reused `_is_permission_error`). One-way only: no `request()`, no reply, no callback to observe — `_probe`'s own `flush(timeout=2)` plus the shared bounded `_settle_deny` follow-up poll (Design Note item 4) govern timing | `request_reply_flows[i]` + active-identity list (`T` selection) | `test_flow_round_trip_denies_non_flow_pair[<requester>-<responder>]` |
...
| N4/N5 timeout duration | N4 (round-trip): existing `request()` timeout idiom, short and fixed. N5 (one-way publish check, no round-trip, no callback to await): governed by reused `_probe`'s own `flush(timeout=2)`, plus a bounded `_settle_deny` follow-up poll (5 × 0.2s = up to 1s, entered only when the immediate check reported "accepted") shared with N2/N3 — see Design Note item 4. Neither `_probe`'s flush timeout nor the follow-up poll bound is made configurable. |

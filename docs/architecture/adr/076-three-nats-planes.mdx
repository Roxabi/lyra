...
| `factory.clipool.heartbeat` | Health (control-plane) | Internal liveness of a subprocess runner. Not user-facing, not scoped to a conversation. Lives alongside `lyra.{voice,llm,image}.heartbeat` and `factory.system.ready` — a control-plane that may eventually become its own catalogued plane but is not the same shape as Typing. |

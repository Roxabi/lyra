...
`discord.Client` must be first in `DiscordAdapter(discord.Client, OutboundAdapterBase)`. See `src/factory/outbound/AGENTS.md` — "OutboundAdapterBase has NO `__init__`".
...
→ `src/factory/outbound/AGENTS.md` for stage invariants and Format-vs-I/O split decision.
...
`src/factory/inbound/AGENTS.md`. Per-platform helpers (audio, voice, thread

"""InboundBus — per-platform inbound queues with feeder tasks and a staging queue.
...
class InboundBus:
    """Per-platform inbound queues + staging queue consumed by Hub.run().
    ...
    def put(self, platform: Platform, msg: Message) -> None:
        ...
        and dropping the message on QueueFull.
        """
    ...
    def task_done(self) -> None:
        """Notify staging queue that the current message was processed."""
    ...
    async def start(self) -> None:
        ...
        for platform, queue in self._queues.items():
            task = asyncio.create_task(
                self._feeder(platform, queue), name=f"inbound-feeder-{platform.value}"
            )

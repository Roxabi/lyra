...
def test_tg_start_typing_enabled_delegates_to_publisher(monkeypatch: Any) -> None:
...
def test_tg_cancel_typing_enabled_delegates_to_publisher(monkeypatch: Any) -> None:
    ...
    assert scope.bot_id == "main"
    assert scope.scope_id == 100
    assert scope.trace_id == "trace_tg_456"
...
def test_tg_start_typing_enabled_no_publisher_is_noop(monkeypatch: Any) -> None:
...
def test_tg_cancel_typing_enabled_no_publisher_is_noop(monkeypatch: Any) -> None:
...
def test_tg_cancel_typing_disabled_calls_legacy_path(monkeypatch: Any) -> None:
...
def test_tg_no_publisher_calls_when_flag_false(monkeypatch: Any) -> None:
    ...
    with patch.object(adapter._typing, "start"):
        adapter._start_typing(222)
    with patch.object(adapter._typing, "cancel"):
        adapter._cancel_typing(222)

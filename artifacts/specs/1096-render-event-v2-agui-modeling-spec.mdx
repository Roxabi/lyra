...
[NEEDS CLARIFICATION: should StreamProcessor emit BOTH v1 and v2 events for the same source LlmEvent during coexistence (dual emission), OR emit only v2 with v1 still available at the type level for receivers? Dual emission doubles wire traffic but provides ironclad parity test ground truth.]

[NEEDS CLARIFICATION: `runId` semantics — equal to `trace_id` verbatim (simplest, observability-aligned), or a fresh UUID with `trace_id` carried as `metadata.trace_id` (allows multiple runs per trace, e.g., interrupt+resume)? Recommendation lean: verbatim `trace_id` for now; revisit if interrupt+resume becomes a feature.]

[NEEDS CLARIFICATION: `message_id` for Text/Reasoning triplets — one per turn (single text block expected), or new id per text block (allows interleaved text↔tool↔text patterns natural in agentic flows)? Claude CLI behavior on this is U2 from analysis — needs CLI capture to confirm.]
...
| Ambiguity budget (≤5 χ items) | ✓ 3 χ items |

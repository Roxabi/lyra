...
`hub_standalone.py` wires the sink. `bootstrap/factory/wiring_helpers.py` (`lyra start` unified
bootstrap) also wires `JetStreamAuditSink` at line 309 — the gap noted in issue #855 is
resolved.
...
- Unified bootstrap (`lyra start`) gap (#855) is resolved — `wiring_helpers.py:309` wires
  `JetStreamAuditSink` in the unified factory. Both `hub_standalone.py` and `lyra start`

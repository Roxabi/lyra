...
**Quadlet implementation (post-Commit 1B.2/1B.3):** In Podman Quadlet units, Invariant 4 is enforced by `Secret=<secret-name>,type=mount,target=<name>.seed` + `Environment=NATS_NKEY_SEED_PATH=/run/secrets/<name>.seed`. Secret names follow S8 convention (`lyra-nats-<role>`): `lyra-nats-hub`, `lyra-nats-telegram`, `lyra-nats-discord`, `lyra-nats-clipool`. If the Podman secret is absent, the container fails to start — structurally equivalent to the fail-fast mandate above.

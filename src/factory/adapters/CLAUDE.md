# src/factory/adapters/ — Channel Adapters
...
Role interface (Fowler) — the hub's view of every platform adapter: inbound normalization, outbound delivery, audio, attachments. See `src/factory/core/hub/hub_protocol.py` for the full Protocol definition.
...
`discord.Client` must be first in `DiscordAdapter(discord.Client, OutboundAdapterBase)`. See `src/factory/outbound/CLAUDE.md` — "OutboundAdapterBase has NO `__init__`".

## OutboundFormatter / ThrottleCapability / OutboundErrorHandler stages (`src/factory/outbound/`)
...
→ `src/factory/outbound/CLAUDE.md` for stage invariants and Format-vs-I/O split decision.
...
Inbound stages (parse, route, session, dispatch) live in `src/factory/inbound/` — see
`src/factory/inbound/CLAUDE.md`. Per-platform helpers (audio, voice, thread

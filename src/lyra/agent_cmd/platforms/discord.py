"""lyra agent discord — Discord bot management CLI commands."""
...
discord_app = typer.Typer(name="discord", help="Manage Discord bots.")
agent_app.add_typer(discord_app, name="discord")


@discord_app.command(name="list")
def list_() -> None:
    """List all Discord bots."""
    _commands._list("discord")


@discord_app.command(name="show")
def show(bot_id: str = typer.Argument(..., help="Bot ID to show.")) -> None:
    """Show full config for a Discord bot."""
    _commands._show("discord", bot_id)


@discord_app.command(name="add")
...
@discord_app.command(name="edit")
def edit(bot_id: str = typer.Argument(..., help="Bot ID to edit.")) -> None:
    """Interactively edit a Discord bot."""
    _commands._edit("discord", bot_id)


@discord_app.command(name="patch")
def patch(  # noqa: PLR0913
    ...
    """Patch a single field of a Discord bot."""
    kwargs = {
        "agent": agent,
    ...
    _commands._patch("discord", bot_id, **kwargs)


@discord_app.command(name="remove")
def remove(
    bot_id: str = typer.Argument(..., help="Bot ID to remove."),
    yes: bool = typer.Option(False, "--yes", "-y", help="Skip confirmation."),
) -> None:
    """Remove a Discord bot."""
    _commands._remove("discord", bot_id, yes)


@discord_app.command(name="assign")
def assign(
    bot_id: str = typer.Argument(..., help="Bot ID to assign."),
    agent: str = typer.Option(..., "--agent", help="Agent name to assign."),
) -> None:
    """Assign an agent to a Discord bot."""
    _commands._assign("discord", bot_id, agent)


@discord_app.command(name="unassign")
def unassign(bot_id: str = typer.Argument(..., help="Bot ID to unassign.")) -> None:
    """Unassign the agent from a Discord bot."""
    _commands._unassign("discord", bot_id)


@discord_app.command(name="validate")
def validate(bot_id: str = typer.Argument(..., help="Bot ID to validate.")) -> None:
    """Validate a Discord bot configuration."""
    _commands._validate("discord", bot_id)

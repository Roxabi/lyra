...
from factory.core.stores.pairing_config import PairingConfig
...
def _load_raw_config(config_path: str | None = None) -> dict[str, Any]:
    ...
    Resolution: explicit arg → $FACTORY_CONFIG → $ROXABI_FACTORY_DIR/config.toml
...
def _load_messages(language: str = "en") -> MessageManager:
    ...
    if not path_str.endswith(".toml"):
        log.warning(
            "FACTORY_MESSAGES_CONFIG %r does not end with .toml — "
            "ignoring, using bundled",
            path_str,
        )

...
imageCLI#50 (closed 2026-04-15) shipped `imagecli nats-serve`, a satellite process that subscribes to `factory.image.generate.request` using `NatsAdapterBase` from `roxabi-nats`. The satellite is fully operational; it is the lyra-side contract documentation that is missing.

ADR-047 (§ imageCLI migration state) explicitly notes the remaining lyra-side work: contract ADR for `factory.image.*`, `src/lyra/nats/nats_image_client.py`, and the `image-worker` nkey in `gen-nkeys.sh`. This ADR delivers the first item.

As with the voice domain (ADR-044), the wire contract must be frozen so that both sides can evolve safely. This ADR freezes the subjects, payload schemas, error codes, max-payload cap, and sanitization obligations for all `factory.image.*` traffic.

### Heartbeat subject reconciliation

Issue #754 originally referenced the heartbeat subject as `factory.image.generate.heartbeat`. The shipped satellite at `imagecli/src/imagecli/nats/adapter.py:18` defines:

```python
HEARTBEAT_SUBJECT = "factory.image.heartbeat"
```

This ADR aligns to the **shipped subject** (`factory.image.heartbeat`). The `generate` segment is omitted because heartbeats are worker-level liveness signals, not tied to a single verb. This is consistent with the naming convention in ADR-035 and the voicecli heartbeat pattern in ADR-044.

Cross-references: imageCLI#50 (satellite implementation), `git show cc05650ee^:artifacts/specs/754-lyra-image-domain-integration-spec.mdx` (lyra-side spec).
...
### Request — `factory.image.generate.request`
...
### Heartbeat — `factory.image.heartbeat`
...
| `subject` | string | ✓ | Always `factory.image.generate.request` |
...
**Why these subjects.** `factory.image.generate.request` follows the `<namespace>.<capability>.<action>.<verb>` shape from ADR-035. The heartbeat subject uses `factory.image.heartbeat` (no `generate` qualifier) because it is a worker-level liveness signal, not scoped to a single verb. This matches the shipped satellite code (`adapter.py:18`) and is consistent with future additional image-domain verbs (e.g. `factory.image.upscale.request`) sharing the same worker heartbeat.
...
- **ADR-035** — NATS subject naming convention. `factory.image.generate.request` and `factory.image.heartbeat` follow the `<namespace>.<capability>.<action>.<verb>` and `<namespace>.<capability>.<verb>` shapes respectively.
...
- Spec: `git show cc05650ee^:artifacts/specs/754-lyra-image-domain-integration-spec.mdx`

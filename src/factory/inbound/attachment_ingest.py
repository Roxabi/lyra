...
``factory.adapters``.  BlobStorePort is imported from ``factory.core.ports.blobstore``
...
# Mirrors FACTORY_MAX_AUDIO_BYTES (TG getFile 20 MiB cap) — raised via env for
# large CDN→PUT transfers (#1552 / parent Open-Q1).
_raw_max_ingest = os.environ.get("FACTORY_MAX_ATTACHMENT_INGEST_BYTES")
try:
    ...
    raise ValueError(
        f"FACTORY_MAX_ATTACHMENT_INGEST_BYTES must be an int, got {_raw_max_ingest!r}"
    ) from None

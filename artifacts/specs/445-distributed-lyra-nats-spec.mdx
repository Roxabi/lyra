...
**NATS is opt-in, not the default.** A mono-machine setup must work with zero NATS configuration. Lyra is an open project — contributors and single-machine users must not be forced to provision NATS just to run the bot.

- `provision.sh` base install does not include NATS — NATS setup is a separate opt-in step (`make nats-install` in lyra-stack)
- `NatsLlmDriver` only registers in `ProviderRegistry` when `NATS_URL` env var is set
- Health probe omits NATS entirely when not configured (`nats` key absent, not `degraded`)
- `lyra-stack.service` has no hard ordering dependency on `nats.service` unless opted in
...
> **Status: done** (#448, merged 2026-03-30 in lyra-stack)
...
5. NATS config, cert generation script, and nkey generation script live in `lyra-stack` repo (infrastructure)
6. nkey seed files stored in `/etc/nats/nkeys/` (mode 0600, root-owned). Application processes read seed path from environment variables.

#### A2b: Make NATS install opt-in (#469)

NATS was baked unconditionally into `provision.sh` as part of A2. This must be corrected before Slice B ships.

1. Extract NATS block from `provision.sh` — base provisioning runs without it
2. `make nats-install` in `lyra-stack` runs the NATS block (idempotent)
3. `lyra-stack.service` has no `After=nats.service` / `Requires=nats.service` by default
4. Document `make nats-install` in `lyra-stack/README.md` under "Multi-machine setup"

#### A3: NATS health probe (#449)

1. Health probe runs **only** when `NATS_URL` env var is set — skipped entirely otherwise
2. `NATS_URL` set + reachable → `"nats": "ok"`
3. `NATS_URL` set + unreachable → `"nats": "unreachable"` (overall status: `degraded`)
4. `NATS_URL` absent → `nats` key omitted from health response (not `degraded`)

...
- Slice 3 carries a real unknown: the Claude CLI subprocess `--include-partial-messages` `InputJsonDelta` JSON shape is unverified in Lyra's code path (current `cli_streaming_parser` drops args). Slice 3 requires a sub-analysis before its `/spec` runs.

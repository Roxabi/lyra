...
| Scenario | Behavior |
|----------|----------|
| Missing `[auth.telegram]` or `[auth.discord]` at startup | Log warning, return `None` — that adapter is disabled. Both missing → `SystemExit` |
| Missing `[auth.cli]` section | Return fixed-OWNER middleware (CLI is always local/trusted) |
| Missing `lyra.toml` entirely | `_load_raw_config()` returns `{}` → both auth sections missing → `SystemExit` |
...
- [ ] `from_config(raw, "telegram")` returns `None` + logs warning when `[auth.telegram]` absent
- [ ] `from_config(raw, "cli")` returns fixed-OWNER middleware when `[auth.cli]` absent
- [ ] `from_config(raw, "telegram")` raises `ValueError` when `default` value is invalid
...
- [ ] Service starts successfully with only `[auth.telegram]` in `lyra.toml` (discord adapter disabled)
- [ ] Service starts successfully with only `[auth.discord]` in `lyra.toml` (telegram adapter disabled)
- [ ] Service raises `SystemExit` when both `[auth.telegram]` and `[auth.discord]` are absent

...
Additionally, non-interactive PATH is broken: `.venv/bin` for `factory-acl`, `uv`, and `lyra` is not available in headless contexts, causing partial deploy aborts mid-run.

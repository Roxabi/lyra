...
- [ ] Every TTS response mock in `tests/nats/test_nats_tts_client.py` with `"ok": True` includes `"duration_ms"` as an int. Verified: `python - <<'PY' ...` one-liner (or equivalent pytest assertion) scans the file and exits non-zero on any `"ok": True` literal missing a sibling `"duration_ms"`.
...
- [ ] `uv run pytest tests/nats/` exits 0.

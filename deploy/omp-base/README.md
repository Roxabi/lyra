...
The tag is the omp version without the `v` prefix (e.g. `16.1.23`). Tags are **immutable** —
...
```dockerfile
COPY --from=ghcr.io/roxabi/factory-omp-base:16.1.23 /opt/omp/omp /usr/local/bin/omp
```

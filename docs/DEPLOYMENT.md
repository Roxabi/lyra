...
NATS_NKEY_SEED_PATH=/etc/nats/nkeys/hub.seed  # nkey seed file for NATS auth (generated by deploy/nats/gen-nkeys.sh)
...
The nkey seed file is generated once by `sudo deploy/nats/gen-nkeys.sh`. By default it is owned `root:root` with mode `0600`, so the service user needs read access — e.g. `chmod 640 /etc/nats/nkeys/hub.seed` and add the user to the `nats` group. If `NATS_NKEY_SEED_PATH` is unset, Lyra connects to NATS without authentication (local dev mode).

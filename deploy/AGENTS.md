...
| `code-only` | **Only** field 0 (`git-head`) differs — every artifact (units / auth / voice / image digests) is unchanged | `converge.sh` resolves it with a `git diff --name-only` over the commit range, in two tiers: **(1) inert** (`_code_change_is_inert`) — every changed path is non-runtime (`docs/`, `tests/`, `artifacts/`, `.github/`, `*.md`, …) → record the new stamp and **skip** (no restart); **(2) image-carried** (`_code_change_is_image_carried`) — every changed path is inert ∪ `src/`/`packages/`/`apps/`/`brand/` (code that reaches the fleet only inside `factory:staging-svc`/`factory:staging`) → record the stamp and **skip the pre-image restart**; the restart is deferred to `factory-post-autoupdate`'s digest converge (fields 4–5) once publish.yml ships the image, killing the "2 full-fleet restarts per code merge" double-bounce. Any host-carried path (`deploy/`, `tools/`, lockfiles, …), or an undecidable diff (missing commit) → fall through to **structural**. Fail-safe by construction: a mis-classification can only over-restart (the old behaviour), never silently under-restart. Requires content-addressed images (provenance/revision removed, lot 4a) so a docs-only commit does not move fields 4–5. |

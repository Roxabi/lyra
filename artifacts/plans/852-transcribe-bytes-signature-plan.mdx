...
flowchart TD
    subgraph Caller_Layer
        ...
        AA[anthropic_agent.py<br/>reads bytes from tmp_path]
    end
...
- `anthropic_agent.py` is scheduled for removal on another worktree (#666). Apply minimal 3-line adaptation here to keep CI green; full drop handled elsewhere.
...
|---|---|---|
| backend-dev | 6 | `src/lyra/nats/nats_stt_client.py`, `src/lyra/stt/__init__.py`, `src/lyra/core/hub/middleware/middleware_stt.py`, `src/lyra/agents/simple_agent_prompts.py`, `src/lyra/agents/anthropic_agent.py` |
| tester | 3 | `tests/nats/test_nats_stt_client.py`, `tests/integration/test_voice_end_to_end.py`, `tests/core/conftest.py`, `tests/core/hub/test_message_pipeline_stt.py`, `tests/agents/test_anthropic_agent.py` |
...
**T8 [GREEN] · backend-dev · difficulty 1**
- Files: `src/lyra/agents/anthropic_agent.py`
- Line 154: replace `await self._stt.transcribe(tmp_path)` with 3-line adaptation: read bytes + mime, call `transcribe(bytes, mime)`. Mechanical only — no logic change. File is scheduled for removal in #666.
- Verify: `uv run pyright src/lyra/agents/anthropic_agent.py`
- Expected: clean
- Spec trace: SC5
- Dependencies: T4

**T9 [GREEN] · tester · difficulty 2**
- Files: `tests/integration/test_voice_end_to_end.py` (2 fakes), `tests/core/conftest.py` (1 fake), `tests/core/hub/test_message_pipeline_stt.py` (3 fakes), `tests/agents/test_anthropic_agent.py` (mock sites if any call `.transcribe`).
- Update all fake `async def transcribe(self, path: Any)` signatures to `(self, audio: Any, mime: Any)`. Update any direct `.transcribe(...)` invocations in tests to pass bytes + mime.
- Verify: `uv run pytest tests/integration tests/core/hub tests/agents -x`
- Expected: all green
- Spec trace: SC6
- Dependencies: T6, T7, T8

**T10 [RED-GATE] · tester · difficulty 1**

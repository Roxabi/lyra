...
import {
  ...
  mockAgentsList,
  mockConnectorInstallations,
  mockConnectors,
  mockFleet,
  mockGithubInstallUrl,
  mockJobs,
  mockOpsHealth,
  mockOpsLogs,
  mockPipelineRuns,
  ...
  upsertMockConnectorInstallation,
  verifyJobsStreamToken,
  verifyPipelineStreamToken,
  verifyStreamToken,
} from "./fixtures";
...
async function handleMockApi(
  ...
  try {
    ...
    // POST /api/bff/jobs/cancel
    if (method === "POST" && path === "/api/bff/jobs/cancel") {
      const body = (await readJsonBody(req)) as { job_id?: string };
      sendJson(res, 200, {
        accepted: true,
        message: `Mock cancel for ${body.job_id ?? "job"}`,
      });
      return;
    }

    // POST /api/bff/jobs/stream-token
    if (method === "POST" && path === "/api/bff/jobs/stream-token") {
      sendJson(res, 200, { stream_token: mintJobsStreamToken() });
      return;
    }

    // GET /api/bff/jobs/stream (SSE)
    if (method === "GET" && path === "/api/bff/jobs/stream") {
      const token = url.searchParams.get("token");
      if (!verifyJobsStreamToken(token)) {
      ...
      res.setHeader("Cache-Control", "no-cache");
      res.setHeader("Connection", "keep-alive");
      res.write(`data: ${JSON.stringify({ type: "snapshot", jobs: mockJobs().jobs })}\n\n`);
      res.end();
      return;
    }

    // POST /api/bff/pipeline/stream-token
    if (method === "POST" && path === "/api/bff/pipeline/stream-token") {
      sendJson(res, 200, { stream_token: mintPipelineStreamToken() });
      return;
    }

    // GET /api/bff/pipeline/stream (SSE)
    if (method === "GET" && path === "/api/bff/pipeline/stream") {
      const token = url.searchParams.get("token");
      if (!verifyPipelineStreamToken(token)) {
        sendJson(res, 403, { detail: "invalid stream token" });
        return;
      }
      res.statusCode = 200;
      ...
      res.setHeader("Cache-Control", "no-cache");
      res.setHeader("Connection", "keep-alive");
      res.write(`data: ${JSON.stringify({ type: "snapshot", runs: mockPipelineRuns().runs })}\n\n`);
      res.end();
      return;
    }

    // GET /api/bff/spans
    if (method === "GET" && path === "/api/bff/spans") {
      sendJson(res, 200, mockSpans());
      return;
    }
    ...
    if (method === "GET" && path === "/api/bff/connectors") {
      ...
      sendJson(res, 200, mockConnectors());
      return;
    }
    ...
    if (method === "GET" && path === "/api/bff/connectors/github/install-url") {
      ...
      sendJson(res, 200, mockGithubInstallUrl());
      return;
    }

    const connectorInstallationsMatch = path.match(
      /^\/api\/bff\/connectors\/([^/]+)\/installations$/,
    );
    if (connectorInstallationsMatch) {
      ...
      if (method === "GET") {
        sendJson(res, 200, mockConnectorInstallations(connector));
        return;
      }
      if (method === "POST") {
        const body = (await readJsonBody(req)) as { external_id?: string; factory_tenant?: string };
        const externalId = String(body.external_id ?? "").trim();
        if (!externalId) {
          sendJson(res, 422, { detail: "external_id required" });
          return;
        }
        upsertMockConnectorInstallation(connector, externalId, body.factory_tenant ?? "roxabi");
        sendJson(res, 200, { ok: true });
        return;
      }
    }

    const connectorInstallDelete = path.match(
      /^\/api\/bff\/connectors\/([^/]+)\/installations\/([^/]+)$/,
    );
    if (connectorInstallDelete && method === "DELETE") {
      ...
      deleteMockConnectorInstallation(connector, externalId);
      sendJson(res, 200, { ok: true });
      return;
    }

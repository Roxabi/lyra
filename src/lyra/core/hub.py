...
class Hub:
    ...
    def register_binding(
        self, platform: Platform, bot_id: str, scope_id: str,
        agent_name: str, pool_id: str,
    ) -> None:
    ...
    async def circuit_breaker_drop(self, msg: InboundMessage) -> bool:
        return await hub_dispatch.circuit_breaker_drop(self, msg)
    ...
    async def dispatch_streaming(
        ...
        await hub_dispatch.dispatch_streaming(self, msg, chunks, outbound)

    async def dispatch_attachment(
        self, msg: InboundMessage, attachment: OutboundAttachment,
    ) -> None:
        await hub_dispatch.dispatch_attachment(self, msg, attachment)

    async def dispatch_audio(
        self, msg: InboundMessage, audio: OutboundAudio,
    ) -> None:
        await hub_dispatch.dispatch_audio(self, msg, audio)

    async def dispatch_audio_stream(
        self, msg: InboundMessage, chunks: AsyncIterator[OutboundAudioChunk],
    ) -> None:
        await hub_dispatch.dispatch_audio_stream(self, msg, chunks)

    async def dispatch_voice_stream(
        self, msg: InboundMessage, chunks: AsyncIterator[OutboundAudioChunk],
    ) -> None:
        await hub_dispatch.dispatch_voice_stream(self, msg, chunks)

    async def _dispatch_pipeline_result(
        self, msg: InboundMessage, result: PipelineResult,
    ) -> None:
        await hub_dispatch.dispatch_pipeline_result(self, msg, result)

...
def test_session_snapshot_is_frozen():
    ...
    from factory.core.memory.memory import SessionSnapshot
...
async def mm():
    ...
    from factory.core.memory.memory import MemoryManager
...
async def test_connect_calls_db_connect():
    ...
    with (
        patch("factory.core.memory.memory.AsyncMemoryDB") as MockDB,
        patch("factory.core.memory.memory.apply_schema_compat") as mock_schema,
    ):
...
async def test_close_calls_db_close():
    ...
    with (
        patch("factory.core.memory.memory.AsyncMemoryDB") as MockDB,
        patch("factory.core.memory.memory.apply_schema_compat"),
    ):
        mock_db_instance = AsyncMock()
        MockDB.return_value = mock_db_instance

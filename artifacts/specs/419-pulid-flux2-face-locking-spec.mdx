...
**Recommended shape: Shape A** — ComfyUI + `iFayens/ComfyUI-PuLID-Flux2` on Machine 2
...
### OS split — now vs later

Machine 2 is currently **WSL2** (Windows + WSL2). Pop!_OS is a planned migration.

| Phase | Slice | OS | GPU needed? |
|-------|-------|----|------------|
| Prep | Slice 0 — docs, templates, directory structure | **WSL2 — do now** | ❌ |
| GPU work | Slices 1–3 — install, workflow, campaign | **Pop!_OS — defer** | ✅ |

ComfyUI technically runs on WSL2 with Blackwell, but adds significant extra friction
(WSL2 CUDA passthrough, port forwarding for `localhost:8188`, filesystem path bridging,
higher chance of PyTorch nightly compatibility issues). Given Pop!_OS migration is upcoming,
the GPU work is better deferred — the setup investment would need to be redone anyway.
...
### Phase 0 — Documentation Prep (WSL2, now, ~0.5 day, no GPU)

Mickael creates the directory structure and documentation scaffolding that the GPU phases
will fill in:

- `brand/workflows/` — created with `lyra-face-lock-README.md` stub (node chain, tuning
  table, VRAM notes, AntelopeV2 install steps, GPU mutex warning)
- `brand/prompts/avatar-face-locked/` — created with 2–3 starter prompt `.md` files using
  the extended face-lock frontmatter (see Breadboard S0)
- `brand/AVATAR-PLAYBOOK.md` — `## 6. Face-Lock Workflow (PuLID Flux2)` section added
  with structure headings and placeholder content to be filled during Phase 2–3
- `.gitattributes` updated: `git lfs track "brand/concepts/avatar-final/face-locked/*.png"`

All of these are text-only changes that can be committed from WSL2 today.

### Phase 1 — Environment Setup (~0.5 day, Pop!_OS)

Mickael installs ComfyUI on Machine 2 after migrating to Pop!_OS. RTX 5070 Ti uses CUDA
sm_120 — PyTorch nightly cu128 is required (not stable PyTorch); xformers must not be
installed. The exact PyTorch wheel URL used is recorded in the README for reproducibility.
...
### S0 — Documentation prep (WSL2, now)

| Element | Action | Output |
|---------|--------|--------|
| `brand/workflows/` | Create directory | git-tracked |
| `brand/workflows/lyra-face-lock-README.md` | Write stub: node chain, tuning table, VRAM notes, AntelopeV2 steps, GPU mutex | committed stub |
| `brand/prompts/avatar-face-locked/` | Create directory + 2–3 starter prompt `.md` files | see frontmatter template below |
| `brand/AVATAR-PLAYBOOK.md` | Add `## 6. Face-Lock Workflow (PuLID Flux2)` with placeholder sections | git-tracked |
| `.gitattributes` | `git lfs track "brand/concepts/avatar-final/face-locked/*.png"` | LFS rule committed |

### S1 — ComfyUI + PuLID environment (Pop!_OS)
...
### S2 — Face-lock workflow (Flux2-native nodes, Pop!_OS)
...
### S3 — Variation campaign + documentation (Pop!_OS)
...
| # | Slice | OS | Deliverable | Demo-able? |
|---|-------|----|-------------|-----------|
| 0 | **Prep** — docs, templates, LFS rule | **WSL2 — now** | `brand/workflows/` stub, `brand/prompts/avatar-face-locked/` with 2–3 `.md` files, AVATAR-PLAYBOOK § 6 placeholder, `.gitattributes` LFS rule | ✅ Text-only, no GPU needed |
| 1 | **Environment** — ComfyUI + PuLID on Machine 2, smoke-test passes | Pop!_OS | `localhost:8188` loads flux2-klein + PuLID node, smoke-test image generated (JIT warm) | ✅ ComfyUI UI accessible, model loaded |
| 2 | **Workflow** — face-lock workflow built, quality-gated, committed | Pop!_OS | `lyra-face-lock.json` + README filled, ≥4/5 test outputs pass face identity check | ✅ Show 5 samples beside reference |
| 3 | **Campaign** — ≥20 variations generated, prompt files + playbook filled | Pop!_OS | ≥5 `.md` files, ≥20 PNGs in Git LFS, AVATAR-PLAYBOOK § 6 complete | ✅ Gallery of diverse Lyra variations |
...
### Slice 0 — WSL2, now
- [ ] `brand/workflows/lyra-face-lock-README.md` committed — contains node chain spec, tuning table, VRAM notes, AntelopeV2 install steps, and GPU mutex warning (stub; to be verified on Pop!_OS)
- [ ] ≥2 prompt `.md` files committed to `brand/prompts/avatar-face-locked/` with extended PuLID frontmatter (`engine`, `steps`, `guidance`, `seed`, `pulid_strength`, `pulid_method`, `pulid_start_at`, `pulid_end_at`)
- [ ] `.gitattributes` updated with Git LFS rule for `brand/concepts/avatar-final/face-locked/*.png`
- [ ] `brand/AVATAR-PLAYBOOK.md` contains `## 6. Face-Lock Workflow (PuLID Flux2)` section with placeholder headings

### Slices 1–3 — Pop!_OS, after migration
...
- [ ] ≥20 diverse variations generated covering ≥4 distinct expressions, ≥3 lighting setups, ≥2 head angles, committed to Git LFS
- [ ] `brand/workflows/lyra-face-lock.json` committed and `lyra-face-lock-README.md` fully verified against actual install (PyTorch nightly wheel URL recorded)

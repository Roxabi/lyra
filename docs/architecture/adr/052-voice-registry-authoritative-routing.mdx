...
> **Amendment (2026-05-31, #1278):** The `_walk_registry()` method described in the
> Implementation Notes section was absorbed into `WorkerPoolClient` as part of the
> stage-axis transport extraction (Epic #1277 Phase 1). The routing logic and registry
> contract remain valid; the implementation now lives in
> `src/lyra/transport/worker_pool_client.py` (`WorkerPoolClient.request_with_routing`)
> rather than inline in each hub-side NATS client.
...
2. **NATS queue-group fallback** — If the registry-selected worker timed out or returned no response, the request was re-published to a queue-group subject (e.g., `factory.voice.stt.request`) where any subscribed worker could pick it up.
...
|---|---|
| STT | `factory.voice.stt.request.<worker_id>` |
| TTS | `factory.voice.tts.request.<worker_id>` |
| Image | `factory.image.generate.request.<worker_id>` *(2026-07-04: corrected — was `factory.image.request.<worker_id>`, missing the `generate` segment; see packages/roxabi-contracts/src/roxabi_contracts/image/subjects.py)* |
...
- **Per-worker subscription model.** Workers must subscribe to their specific per-worker subject (`factory.voice.stt.request.<worker_id>`), not a queue-group wildcard. This is already the case for the current implementation; the ADR formalizes it as a rule.
...
- **ADR-049** — `roxabi-contracts` shared schema package (absorbs ADR-044, archived). Defines the request/reply payload structure. This ADR modifies the routing behavior without changing the wire contract.

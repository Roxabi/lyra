---
title: "ADR-071: CliPool Claude OAuth token via type=env Podman-secret exception"
description: Documents the type=env Podman secret exception for CLAUDE_CODE_OAUTH_TOKEN injection into factory-clipool, the threat-model scope that bounds it, and the type=mount fallback if Podman bug #28075 forces a switch.
status: amended
---
...
Related: ADR-055 (Quadlet ecosystem conventions, absorbs ADR-054 archived — the `type=mount` Podman secret pattern lives there now).
Issue: #1105 (this ADR's implementation), #1108 (Podman version verification preflight).

Amended 2026-07-04 (operational claim audit): `lyra→factory` rename reflected in unit names and secret names (see Amendment below).

### Amendment (2026-07-04)

Operational audit found comprehensive `lyra→factory` rename stale claims throughout this ADR. The decision and threat-model scope remain unchanged; all changes are unit-name and secret-name editorial updates affecting operational procedures:

1. **Description** (line 3): `lyra-clipool` → `factory-clipool`.
2. **Decision §1 — unit name** (line 54): `lyra-clipool.container` → `factory-clipool.container` — verify via `deploy/quadlet/factory-clipool.container`.
3. **Decision §1 — secret name** (line 57): `Secret=lyra-claude-oauth` → `Secret=factory-claude-oauth` — verify via `deploy/quadlet/factory-clipool.container:38`.
4. **Threat model — secret mask** (line 69): `lyra-claude-oauth=*******` → `factory-claude-oauth=*******` (podman inspect output, secret name only, value masked).
5. **Threat model — hidepid note** (line 72): `lyra-clipool.container:65-70` → `factory-clipool.container`.
6. **Bootstrap and rotation** (line 80): `~/.lyra/claude-oauth.tok` → `~/.roxabi/factory/claude-oauth.tok` (data dir moved in rename).
7. **Bootstrap and rotation** (line 81): `systemctl --user restart lyra-clipool.service` → `factory-clipool` (Quadlet unit, no `.service` extension).
8. **Verification §#28075** (line 90): `lyra-clipool.container` → `factory-clipool.container`.
9. **Verification outputs** (lines 108, 109, 113, 115, 122): Example `podman inspect lyra-clipool` output and secret names — all `lyra-*` → `factory-*` in examples.
10. **Fallback path ini block** (§Fallback): `Secret=lyra-claude-oauth` → `Secret=factory-claude-oauth` — forward-looking instruction, updated in place.

Verification transcripts (§Verification outcome, recorded 2026-05-07 on M₁) keep the `lyra-*` names they were captured under — read them as `factory-*` today per the mapping above.

## Context

The `claude` CLI subprocess spawned by `lyra-clipool` *(2026-07-04: now `factory-clipool` — see Amendment.)* requires authentication to call Anthropic. Three upstream constraints frame the choice:

1. **OAuth refresh broken in headless subprocess contexts** ([anthropics/claude-code#50743](https://github.com/anthropics/claude-code/issues/50743), closed-as-duplicate, no fix planned). The `accessToken` in `~/.claude/.credentials.json` expires every ~6–8h; the CLI does not auto-refresh in non-TTY invocations. Cloudflare WAF additionally blocks the OAuth refresh endpoint from headless Linux ([#47754](https://github.com/anthropics/claude-code/issues/47754)). Production lyra-clipool *(2026-07-04: now `factory-clipool` — see Amendment.)* fails with `401 authentication_error` every ~6–8h until interactive re-login.
...
ADR-055 (which absorbs the archived ADR-054) establishes `type=mount` as the standard Podman secret delivery pattern for Lyra (NATS nkeys, GitHub App PEM). All existing secrets follow this convention.

## Decisions

### 1. Use `Secret=…,type=env,target=CLAUDE_CODE_OAUTH_TOKEN`

`factory-clipool.container` *(2026-07-04: see Amendment.)* declares:
...
**Rationale for `type=env` over `type=mount`:** the `claude` CLI must receive the token in its environment regardless of upstream delivery — there is no `--token-file` flag. Using `type=mount` would require Python to read `/run/secrets/claude-oauth.tok` and inject the contents into `env` at spawn time, achieving identical `/proc/<claude-pid>/environ` exposure with one extra syscall and a parent-process token lifecycle to manage. The architect's analysis on the consensus panel (`git show eeeb288c2^:artifacts/analyses/1105-claude-oauth-injection-consensus.mdx`) concluded: "mount-then-read achieves *identical* exposure with more indirection. No benefit."
...
- **`podman inspect` value exposure** ([containers/podman#23788](https://github.com/containers/podman/issues/23788), value-leak fixed in PR #23959). Verified on M₁ (Podman 5.7.0, 2026-05-07): `Config.Env` contains an entry keyed by the *secret name* with the value masked — `lyra-claude-oauth=*******` *(2026-07-04: now `factory-claude-oauth` — see Amendment.)*. The actual token value never appears. Note: the entry is keyed by secret name, not by the `target=` alias, so `Config.Env` exposes which Podman secrets are bound but not their contents. Only readable by users with access to the Podman socket (= the daemon user) regardless.
...
`hidepid=2` on the container's `/proc` mount is intentionally NOT set per `lyra-clipool.container:65-70` *(2026-07-04: now `factory-clipool.container` — see Amendment.)* — it would break `podman exec` introspection valuable for ops, and the single-tenant scope makes the `/proc/environ` window already-bounded.
...
- Bootstrap (1×): `claude setup-token > ~/.roxabi/factory/claude-oauth.tok` *(2026-07-04: see Amendment.)* on an interactive workstation, then `make quadlet-secrets-install` (or `CLAUDE_OAUTH_TOKEN_PATH=… deploy/provision.sh`) creates the Podman secret. The provision path validates 0600 mode, strips trailing newlines, and shreds the source file after secret creation.
- Rotation: `deploy/scripts/rotate-claude-oauth.sh /path/to/new.tok` — same pattern, plus `systemctl --user restart factory-clipool` *(2026-07-04: see Amendment.)* to pick up the new env var binding.
...
```ini
Secret=factory-claude-oauth,type=mount,target=claude-oauth.tok,mode=0400,uid=1500,gid=1500
```

...
- **Snippet:** parametrized boundary cases — 74d ok, 75d warn, 89d warn, 90d fail, 91d fail (all with a real log entry); zero log entries → always ok (grace) regardless of mtime, including a synthetic mtime that would be >90d if it were gating; no log entry + no seed file → grace, no informational hint; malformed line skipped; non-active `secret:` ignored
- **Verify:** `uv run pytest tests/scripts/test_check_seed_age.py -q` (expected: FAIL — module doesn't exist)
...
- **Snippet:** `WARN_DAYS = 75`, `FAIL_DAYS = 90`; `def run(matrix, rotation_log_path, seeds_dir, *, today=None) -> tuple[list[str], list[str]]`; `today` defaults to `datetime.now(timezone.utc).date()`; parse `rotation-log.md` via regex matching `operator-log.sh:77`'s exact format, skip header/malformed lines, keep latest date per `secret:`, ignore non-active identities; identity with a log entry → real age gates warn/fail; identity with no entry → grace (always ok), read `{name}.seed` mtime if present and return/print as an informational `NOTE:` that never affects the return value's warn/fail lists
- **Verify:** `uv run pytest tests/scripts/test_check_seed_age.py -q` (expected: PASS)
...
- **Snippet:** `_cmd_check_seed_age(args)` (mirrors `_cmd_check_grants`) lazily imports `scripts.check_seed_age.run`; `ck_seed_age = ck_sub.add_parser("seed-age", ...)` with `--matrix` arg; reads `ROTATION_LOG` env (default `~/.roxabi/factory/rotation-log.md`) and `scripts._modes._seeds_dir()`; prints `WARN:`/`FAIL:`/`NOTE:` to stderr, `sys.exit(1)` iff any failure
- **Verify:** `uv run factory-acl check seed-age --help` exits 0; `grep -q '"seed-age"' scripts/gen_nkeys.py`

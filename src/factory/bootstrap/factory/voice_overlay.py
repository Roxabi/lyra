...
def init_blobstore() -> "BlobStorePort | None":
    ...
    Reads ``FACTORY_BLOBSTORE_URL`` and ``FACTORY_BLOBSTORE_TOKEN_PATH`` once at
    ...
    base_url = os.environ.get("FACTORY_BLOBSTORE_URL", "http://localhost:8449")
    ...
    if _parts.scheme == "http" and host is not None and not _is_loopback(host):
        log.warning(
            "FACTORY_BLOBSTORE_URL %s is non-loopback http — bearer token sent in "
            "cleartext (mitigated by Tailnet WireGuard); prefer https",
            base_url,
        )

    token_path = os.environ.get(
        "FACTORY_BLOBSTORE_TOKEN_PATH",

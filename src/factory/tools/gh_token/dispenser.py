"""Unix socket dispenser for the factory-gh token-mint helper.

Listens on /run/factory-gh-token/dispenser.sock (mode 0660, group
factory-tokenuser) and serves git-credential protocol responses. The dispenser
is the IPC seam between the helper-uid daemon (1501) and the Claude-uid
client (1500); they share the factory-tokenuser group (gid 1502) so the
credential helper / factory-gh shim can connect, but the cache file
/run/factory-gh-token/token.json (mode 0600) remains unreadable to uid 1500.
...
class Dispenser:
    ...
    async def serve(self, sock_path: Path) -> asyncio.AbstractServer:
        ...
        then immediately chmod'd to 0660 so members of factory-tokenuser (gid 1502)

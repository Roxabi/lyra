...
import enDesignSystem from "@/i18n/locales/en/designSystem.json";
...
import frDesignSystem from "@/i18n/locales/fr/designSystem.json";
...
import frSpans from "@/i18n/locales/fr/spans.json";

const STORAGE_KEY = "factory.dashboard-v2.locale";

function readStoredLocale(): string {
  try {
    const stored = localStorage.getItem(STORAGE_KEY);
    if (stored === "en" || stored === "fr") return stored;
    const legacy = localStorage.getItem("factory.dashboard.locale");
    if (legacy === "en" || legacy === "fr") return legacy;
  } catch {
    // private mode
  }
  return "fr";

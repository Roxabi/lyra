...
| N9 | hub NATS grants | hub identity gains subscribe `factory.job.*.progress` + `factory.job.*.result` (+ regen 4 ACL artifacts + hand-mirror fixture) | `deploy/nats/acl-matrix.json` |
...
| 4 | streaming bridge | N2 | omp turn streams tokens live (parity with clipool) |
...
- [ ] `OmpRpcDriver` extends `roxabi_nats.NatsDriverBase` and satisfies `LlmProvider` (`complete`, `is_alive`, `stream`).
...
- [ ] **Hub NATS grants**: hub identity has subscribe grants for `factory.job.*.progress` + `factory.job.*.result`; the 4 generated ACL artifacts are regenerated and the hand-maintained fixture mirrors the diff (`test_v4_render_set_equals_v3_render` green).

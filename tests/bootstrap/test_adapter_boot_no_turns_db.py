"""T22: Adapter boot does NOT open turns.db (#1721).

Tests verify both standalone paths (standalone_discord, standalone_telegram) never
open a TurnStore backed by turns.db.
...
class TestTelegramStandaloneNoTurnsDb:
    ...
    def test_standalone_telegram_source_has_no_turns_db_reference(self) -> None:
        ...
        assert "TurnStore(" not in source, (
            "standalone_telegram opens TurnStore — turns.db must not be used"
        )

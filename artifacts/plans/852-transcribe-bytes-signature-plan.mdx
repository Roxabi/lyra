...
- T1: 8 — [RED] Update test_nats_stt_client.py fixtures & call sites to bytes+mime
- T2: 9 — [GREEN] Rewrite NatsSttClient.transcribe(audio: bytes, mime: str)
- T3: 10 — [GREEN] Remove Path import + _mime_from_suffix from nats_stt_client.py
- T4: 11 — [GREEN] Update STTProtocol + add _mime_from_suffix helper in lyra.stt
- T5: 12 — [RED-GATE] Slice 1 gate — tests/nats green + pyright green
- T6: 13 — [GREEN] Migrate middleware_stt.py to pass bytes+mime directly
- T7: 14 — [GREEN] Adapt simple_agent_prompts._build_audio_text to bytes+mime
- T8: 15 — [GREEN] Update test fakes to new transcribe signature
- T9: 16 — [RED-GATE] Slice 2 + overall gate — full pytest + pyright green

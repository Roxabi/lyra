...
> Note: `frontend-patterns.md` already documents several *per-component* padding workarounds (`pl-6`/`pr-6` edge cells, `Card padding={0}` collapse) — those are symptoms of the systemic cascade-layer bug this file's §3 fixes at the root. Prefer the §3 root rule over adding more per-component padding patches.
...
| # | Layer | Populated by |
|---|---|---|
| 1 | `reset` | (declared, currently unused directly — Astryx's own `reset.css` self-wraps into it) |
...
| 6 | `brand` | `brand/styles.css` (imported with explicit `layer(brand)`) |
...
**The fix (#2148):** the reset now lives in `apps/dashboard/src/index.css` inside `@layer reset` (position 1, below `astryx-base`), and `brand/base.css` carries **no** universal reset. In `reset` it still normalizes non-Astryx elements (nothing lower re-adds their padding) while Astryx's own `astryx-base` padding legitimately wins. Verified: Astryx Button padding `0`→`8px 12px`, Badge `0`→`0 8px`. (Note: layer assignment is by name, so the `@layer reset { … }` block sits *after* the `@import`s in `index.css` — placing it before them would invalidate the imports per the CSS `@import`-first rule.)
...
- put it in `reset`/`base` (positions 1/3, *before* `astryx-base`) so Astryx's own layers can legitimately win over it, or
- scope it away from Astryx subtrees, or
- drop it — Astryx's own `@astryxdesign/core/reset.css` (`index.css:13`) already normalizes box-sizing/margin/padding for everything Astryx renders; a second unscoped reset is redundant risk, not defense in depth.

The reset now lives in `index.css` `@layer reset`; `brand/base.css` intentionally has none (a comment there points here). **Never re-introduce a universal selector into a file that loads into `brand`, `components`, or `utilities`** — put globals in `reset`/`base` (before `astryx-base`) or scope them.
...
**Don't**
- Don't add a universal selector reset (`*`, `*::before`, `*::after`) to any file loaded into the `brand`, `components`, or `utilities` layers — it will beat every Astryx component style regardless of specificity. (See the confirmed `brand/base.css:6-12` bug in §3 — do not replicate it.)
...
- Don't shrink Astryx component padding/sizing via override classes to "fix" the current cramped-button look — that papers over the real bug (§3) instead of fixing it, and will be doubly wrong once §3 is fixed upstream.
...
| Brand reset vs Astryx layers | `brand/base.css`'s universal reset outranks `astryx-base` in the layer order | **See §3 — confirmed root cause, do not replicate** |

...
`lyra-acl check grants` — a CI falsification gate that asserts the NATS subjects the code
...
| `lyra-acl check grants` exit 0 on committed matrix+manifest, OK line | T4, T5 |
...
- `.github/workflows/ci.yml` after the "Check request-reply flows" step (`uv run lyra-check-flows`):
  ...
  - name: Check ACL code coverage (ADR-079)
    run: uv run lyra-acl check grants --matrix deploy/nats/acl-matrix.json --code-subjects deploy/nats/code-subjects.json
  ```
Verify: `uv run lyra-acl check grants` → exit 0 OK line; `uv run lyra-check-grants` → same;
`uv run lyra-acl check --help` lists `grants`.
...
- After Slice 2 (T5): `uv run lyra-acl check grants` exit 0 on real matrix+manifest.

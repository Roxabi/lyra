...
| `lyra.outbound.audio.{platform}.{bot_id}` | hub → adapter | Audio delivery (JetStream `LYRA_OUTBOUND_AUDIO` `MaxAge=24h`, durable pull consumer `outbound-audio-{platform}`, at-least-once + KV dedup `lyra_outbound_audio_sent` — ADR-077) |

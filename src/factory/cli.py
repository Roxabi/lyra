...
Entry points:
    ...
    factory start               → explicit start
    factory --version / -V      → print version
    factory agent create        → interactive agent wizard
    factory agent list          → list all agents
    factory agent validate      → validate agent TOML
    factory config show         → display parsed config.toml
    factory config validate     → validate config.toml

Backward compat:
    factory-agent               → deprecated alias for `factory agent`
"""
...
def _get_version() -> str:
    try:
        return importlib.metadata.version("factory")
...
factory_app.add_typer(agent_app, name="agent")
factory_app.add_typer(config_app, name="config")
factory_app.add_typer(bot_app, name="bot")
...
factory_app.add_typer(adapter_app, name="adapter")
...
def _boot(coro_factory) -> None:
    ...
    Every ``factory <subcommand>`` entry point delegates here so logging
    is always configured before the event loop starts.
    """
    from factory.bootstrap.factory.config import _load_logging_config, _load_raw_config
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"factory {_VERSION}")
        raise typer.Exit()


@factory_app.callback(invoke_without_command=True)
def _root_callback(
    ctx: typer.Context,
...
def config_validate(
    ...
    from factory.config import load_multibot_config
...
def factory_main() -> None:
    """Entry point for the `factory` CLI."""
    factory_app()


def agent_main() -> None:
    """Deprecated entry point — use `factory agent` instead."""
    typer.echo(

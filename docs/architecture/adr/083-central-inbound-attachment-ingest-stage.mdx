...
Amended 2026-07-04 (operational claim audit): audio cap env var is `FACTORY_MAX_AUDIO_BYTES` (ex-`LYRA_MAX_AUDIO_BYTES`) — dated inline label in Consequences.

> The voice path shipped first (`AttachmentIngestStage` wired for Telegram + Discord).
> Remaining delivery (non-audio attachment types, `PENDING_STORE_KEY` retirement, STT worker
> simplification) is tracked in the issues under epic #1537 — slice status lives there, not here.
...
This is precisely the N×M duplication pattern ADR-073 identifies as the primary architectural anti-pattern. The `inbound-no-adapters` importlinter contract (#1287) already forbids `factory.inbound` modules from importing `discord` or `aiogram`, establishing the boundary this ADR formalizes.
...
- **PENDING_STORE_KEY retirement path.** Once eager ingest covers every attachment type at the adapter sites, `PENDING_STORE_KEY` construction drops to zero and the sentinel can be removed from `roxabi_contracts.blob_ref`.
...
- **Discord CDN URL expiry.** Discord CDN URLs expire within minutes. Ingest must be eager (within the pipeline, before bus enqueue). Confirm timing when extending ingest beyond the voice path.

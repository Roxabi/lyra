...
Superseded by ADR-059 — infrastructure layer concept absorbed into the canonical hexagonal/clean model.
...
```
lyra.core ← factory.llm | lyra.nats ← lyra.adapters ← lyra.bootstrap
```
...
Add `src/lyra/infrastructure/` as a new module sitting between `factory.llm | lyra.nats` and `lyra.adapters` in the layering order. Move the 10 SQLite implementation files there. Leave protocols, the JSON test double, and the `PairingConfig` dataclass in `src/lyra/core/stores/`.
...
**1. Extended layering contract** — `lyra.infrastructure` is inserted between `factory.llm | lyra.nats` and `lyra.adapters`:

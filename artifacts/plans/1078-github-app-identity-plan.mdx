...
#### T16: Rotation test — measure ≤10 s downtime on M₁ → tester — **completed (2026-05-06)**
...
- **Verify:** time field ≤10 s; helper container Up + dispenser socket reachable from clipool at end
- **Expected:** ≤10 s end-to-end
- **Result:** **0.578 s** measured on M₁ (Podman 5.7.0). Functional smoke `lyra-gh issue list --repo Roxabi/lyra` returned a real issue post-rotation. Bug uncovered: original script restarted `lyra-clipool` (no-op for sidecar design) — fixed in commit 283a90db to restart `lyra-gh-helper` (PEM consumer, invariant I-5) and gate on dispenser-socket reachability. Audit Decision Log updated.
- **Time:** 5 min · **Difficulty:** 2
- **Traces:** AC ops-#2
- **Phase:** RED-GATE
...
#### T18: RED-GATE V6 — end-to-end mint failure simulation (revoke installation token) → tester — **deferred (2026-05-06) → #1082**
...
- **Status:** Blocked on audit gap F-1 — helper-side NATS publish is not wired (helper has no nkey identity, would require ADR-046 changes). T17 subscriber + contract envelope are in place; only the producer call is missing. Tracked separately in Roxabi/lyra#1082.
...
#### ~~T22: M₂ deploy — provision + restart + smoke push from dev clipool → devops~~ — **cancelled (2026-05-06) → #1081**
...
- **Cancellation:** Single-App collapse made M₁/M₂ identity invariant — T22 added no new spec dimension (same code, image, App). M₂ Podman 4.9.3 is also incompatible with the chosen `Pod=` design (option (a) in audit gap #2). M₁ T12 already validates the full chain end-to-end. Tracked as a follow-up in Roxabi/lyra#1081 with two design options when un-deferred (Podman upgrade vs option-(b) host-bind variant).
...
- T22: 22 — M₂ deploy + smoke push **(cancelled 2026-05-06 → #1081 follow-up)**
- T18: 18 — RED-GATE V6 mint failure E2E **(deferred 2026-05-06 → #1082 follow-up; blocked on F-1)**

...
Incident 2026-05-25/26 : `factory-acl genkeys --regenerate` a régénéré le seed `voice-client` sur M₁ sans propagation à M₂ → ~14h d'auth-fail silencieux côté NATS, voicecli `dictate nats` bloqué. Hotfix manuel par scp.
...
Toute exécution de `factory-acl genkeys --regenerate` se termine soit en succès complet, soit en exit non-zéro avec un manifest scp copiable — jamais en succès silencieux qui laisse un client externe verrouillé.

## Users

- **Primary** : opérateur (Mickael) qui exécute `factory-acl genkeys --regenerate` sur M₁ après modification d'ACL ou rotation
- **Secondary** :
...
`factory-acl genkeys --regenerate --yes` se comporte comme aujourd'hui : génère seeds, écrit auth.conf, exit 0.
...
```
$ sudo factory-acl genkeys --regenerate --yes
...
```
$ sudo factory-acl genkeys --regenerate --yes --ack-external-distribution
[generates seeds]
[writes auth.conf]
...
|---|---|---|---|---|
| U1 | CLI flag | `factory-acl genkeys --regenerate --ack-external-distribution` | `_cmd_genkeys → _mode_regenerate → _mode_full_provision` | `args.ack_external_distribution` (bool) |
...
```
User runs: sudo factory-acl genkeys --regenerate --yes [--ack-external-distribution]
  ↓
...
**Demo** : `sudo AUTH_DIR=/tmp/test-auth factory-acl genkeys --regenerate --yes` sur matrice de test contenant voice-client → exit 2 + manifest sur stderr. Ajouter `--ack-external-distribution` → exit 0 + manifest toujours visible (warning).
...
**Demo** : matrice v3 committée passe `bun factory-acl check retired` + `check flows` + `load_matrix` sans erreur. `docs/ops/nats-authconf-update.md` contient la nouvelle section. `factory-acl genkeys --regenerate --yes` exit 2 sur matrice v3 sans flag d'ack. Avec flag → exit 0.
...
- [ ] `factory-acl genkeys --regenerate --yes` sur matrice avec externals retourne exit code 2 + manifest sur stderr
- [ ] `factory-acl genkeys --regenerate --yes --ack-external-distribution` retourne exit code 0 + manifest toujours visible sur stderr (warning)

...
| Field | Value |
|-------|-------|
| Description | Run the 3 CI ACL checks locally to confirm no regression: `lyra-check-flows`, `lyra-check-acl-retired`, `factory-acl check grants`. |
| Files | CI workflows (read-only) |
| Verify command | `uv run lyra-check-flows && uv run lyra-check-acl-retired && uv run factory-acl check grants --matrix deploy/nats/acl-matrix.json --code-subjects deploy/nats/code-subjects.json` |
...
| SC10: `factory-acl check grants` CI green | ✓ | T4 |

...
1. The adapter bootstrap module pulls `load_bot_token` from the dedicated
   `factory.bootstrap.credentials` module — not from `config.db`.
...
CREDENTIALS = PROJECT_ROOT / "src/factory/bootstrap/credentials.py"
...
def test_credentials_module_reads_from_run_secrets() -> None:
    ...
    assert "/run/secrets" in src or "FACTORY_RUN_SECRETS_DIR" in src, (
...
def test_adapter_and_wiring_use_credentials_module() -> None:
    """The adapter + wiring entry points pull from `factory.bootstrap.credentials`."""
    for f in (STANDALONE_TG, STANDALONE_DC, WIRING):
        src = f.read_text()
        assert "from factory.bootstrap import credentials" in src, (
...
def test_credentials_has_prod_guard_on_env_override() -> None:
    ...
    FACTORY_RUN_SECRETS_DIR when running inside a container or when FACTORY_ENV=prod
    (issue #1304)."""
    src = CREDENTIALS.read_text()
    ...
    assert (
        "FACTORY_ENV=prod" in src or 'os.environ.get("FACTORY_ENV") == "prod"' in src
    ), "credentials.py must check FACTORY_ENV=prod in the production guard"

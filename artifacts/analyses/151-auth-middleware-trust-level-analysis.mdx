...
**Security-critical edge case:** if an auth section is absent, `AuthMiddleware.from_config()` returns `None` and logs a warning — that adapter is disabled (fails closed by omission). If **both** sections are absent, `__main__.py` raises `SystemExit`. A malformed `default` value always raises `ValueError` → `SystemExit`. The system never fails open.

...
key-generation — it asserts `deploy/conf.d/lyra-*.conf` and
`deploy/quadlet/*.container` files wire `NATS_NKEY_SEED_PATH` for
every owner==lyra identity. It only lives in `gen-nkeys.sh` because
it needs `IDENTITIES[]` from `load_matrix`. Shape C splits it into
its own module (`_supervisor.py`).

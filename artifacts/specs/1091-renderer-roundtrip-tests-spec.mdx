...
Audit (see frame) identified 2 uncovered renderers: `deploy/nats/nats.conf` (+ container variant), `deploy/nats/gen-certs.sh`. Plus a missing fast `nats-server -t -c` parse-gate for `factory-acl genkeys` output (current coverage is the slow live-server e2e in `test_parity_e2e.py`).
...
1. Contributor opens PR touching `factory-acl/**`, `deploy/nats/**`, or the workflow/helper script itself.
...
```mermaid
flowchart LR
    R1[factory-acl genkeys] --> N[nats-server -t -c]
...
|---|---|---|---|---|
| nats-server | `nats-server` | `-t -c <file>` | factory-acl, nats.conf, nats-container.conf, gen-certs (TLS stanza) | this issue |
...
| J1 | `factory-acl-parse` job | GHA job | W1 | Generates auth.conf then parses with `nats-server -t -c` |
...
| T1 | `tools/check_renderer_roundtrip.sh` | helper script | `tools/` | Per-renderer subcommands (factory-acl, nats-conf, gen-certs); `set -euo pipefail`; exit non-zero on consumer failure or invariant violation; extensible — adding a new subcommand is the canonical way to register a new renderer |
...
|---|---|---|
| J1 | `factory-acl genkeys --regen-authconf` → `nats-server -t -c $TMPDIR/auth.conf` (via T1) | exit 0 ∧ ∀ nkey: `len==56` ∧ `pubkey(seed)==nkey` ∧ no embedded `\n` in `"…"` |
...
| T1 | dispatches `factory-acl | nats-conf | gen-certs` subcommands; sources nats-server install via F1 | invariant errors print `expected X, got Y` style diff |
...
|---|---|---|---|
| 1 | factory-acl parse-gate | W1 skeleton + F1 + T1 (factory-acl subcommand) + J1 | Inject 62-char nkey into renderer output → J1 fails with `expected 56, got 62` |
...
- [ ] `.github/workflows/renderer-roundtrip.yml` exists, uses SHA-pinned `actions/checkout`, declares `concurrency` with `cancel-in-progress: true`, `permissions: contents: read`, `timeout-minutes: 10`, and triggers on paths: `factory-acl/**`, `deploy/nats/**`, `tools/check_renderer_roundtrip.sh`, `.github/workflows/renderer-roundtrip.yml`
- [ ] Injecting a 62-char nkey into `factory-acl genkeys` output (matching the pre-#1089 bug state) → J1 fails with output containing `expected 56` and `got 62`
...
- [ ] `docs/standards/renderer-roundtrip.md` exists with non-empty sections — Pattern, When to apply, Required shape, Worked example, Failure modes, Reviewer checklist — and Worked example contains the verbatim T1 factory-acl invocation

...
Shape 3 hybrid burn-down in 5 PR slices: Slice 1 tags all 50 untagged BLE001 sites (inventory
honesty); Slices 2–3 narrow `core/pool`, `core/hub`, and remaining core/inbound; Slice 4 narrows
infrastructure stores and classifies adapter/blobstore survivors; Slice 5 updates ADR-073 and closes
#1832. Each slice PR includes before/after grep counts and passes `check_str_exc_bus_bound.sh`.
...
**T6** [P] [infra] Tag 7 infra sites: dlq_router (4), active_jobs_refresher (2), turn_writer/health (1).
Use `DEBT:boundary-broad-catch` — these are drain targets in V4. Phase: GREEN · Spec: SC3.

**T7** [P] [misc] Tag tools/gh_token (4), cli/bot (2), agent_cmd/bots/init (2), bot_display_name (1).
Phase: GREEN · Spec: SC3.

...
**Claude CLI errors**
The default agent uses `claude-cli` backend, which shells out to the `claude` CLI. Make sure you're logged in: run `claude` once to authenticate. Lyra requires a Claude subscription for the `claude-cli` backend. An alternative `ollama` backend is planned for a future phase and not yet available; for multi-provider access today, route through the `nats` backend → llmCLI worker.

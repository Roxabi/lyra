...
1. Operator regenerates `deploy/nats/auth.conf` via `sudo factory-acl genkeys
...
|---|-------|--------|------|--------------|
| 1 | **ACL canonical migration** — flip YAML source for `hub` + `llm-worker` nkeys, regenerate `auth.conf`, add regression test asserting subject set | N1, N2, N3, N4 | `grep -E "lyra\.llm\.(request\|health\.\*)" deploy/nats/auth.conf` returns nothing; positive grep for canonical subjects | ACL YAML source (under `factory-acl genkeys`), `deploy/nats/auth.conf`, 1 regression test |

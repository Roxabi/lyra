...
**Production**: Sixteen active Quadlet containers on M₁ (`deploy/quadlet.toml` SSoT) share a single NATS server (`factory-nats`). Core factory: `factory-hub`, chat adapters (`factory-telegram`, `factory-discord`, `factory-dashboard`), workers (`factory-clipool`, `factory-omp`), ingress (`factory-ingress`, `factory-cloudflared`), and infra (`factory-gh-helper`, `factory-turn-writer`, `factory-blobstore`). Observability: `factory-loki`, `factory-promtail`, `factory-otel` (Langfuse bundle and legacy `factory-otel-collector` disabled in manifest).

...
Intended use: activate via ``FACTORY_DB=json`` env var or instantiate directly in
...
class JsonAgentStore:
    ...
    Mirrors the public interface of :class:`~factory.core.agent_store.AgentStore`
    so it can be swapped in transparently anywhere the
    :class:`~factory.core.agent_store_protocol.AgentStoreProtocol` is expected.

...
async def process_one(msg: InboundMessage, agent: AgentBase, pool: Pool) -> None:
    ...
    # Inject voice modality before _original_msg is captured.
    ...
    if _enriched_msg is None:
        return  # pre-hook surfaced error and dispatched
    msg = _enriched_msg
...
async def _run_processor_pre(
    ...
    Returns (enriched_msg, processor) on success, (None, None) if error was dispatched.
    """
    _processor = None
...
async def _process_non_streaming(
    ...
    """Dispatch a non-streaming Response.

    Records circuit success, updates session_id, attaches turn-log callback, dispatches.
    """
...
async def _process_streaming(  # noqa: PLR0913 — 6 cohesive params, no natural grouping
    ...
    """Dispatch a streaming result with capture, turn logging, and post-hook."""
    _iter = result
